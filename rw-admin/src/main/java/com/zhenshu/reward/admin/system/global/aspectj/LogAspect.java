...
import com.zhenshu.reward.common.constant.enums.HttpMethod;
...
import com.zhenshu.reward.common.constant.enums.BusinessStatus;
...
public class LogAspect {
    ...
    protected void handleLog(final JoinPoint joinPoint, Log controllerLog, final Exception e, Object jsonResult) {
        try {
            ...
            // 保存数据库
            redisMQTemplate.send(StreamMessage.create(operLog, RmConstants.RECORD_OPER));
        } catch (Exception exp) {
            // 记录本地异常日志
            log.error("==前置通知异常==");
        ...
        if (paramsArray != null && paramsArray.length > 0) {
            for (Object o : paramsArray) {
                if (StringUtils.isNotNull(o) && !isFilterObject(o)) {
                    try {
                        Object jsonObj = JsonUtil.toJson(o);
